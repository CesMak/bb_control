...
%Energie Body
T_b = 0.5*m_b*((r_s*phi_dot)^2+2*r_s*l*phi_dot*theta_dot*cos(theta))+0.5*(I_b)*theta_dot^2;
V_b = m_b*g*l*cos(theta);

% Energie Wheel
T_w = 0.5*m_w*((r_s*phi_dot)^2+2*r_s*(r_s+r_w)*phi_dot*theta_dot*cos(theta)+(r_s+r_w)^2*theta_dot^2)+0.5*I_w*((r_s/r_w)*(phi_dot-theta_dot))^2;
