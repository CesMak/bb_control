...
%Matrizen
M_y=[m_tot*(r_s)^2+I_s+(r_s/r_w)^2*I_w, -(r_s/(r_w)^2)*r_tot*I_w+gamma*r_s*cos(theta_y);
     -(r_s/(r_w)^2)*r_tot*I_w+gamma*r_s*cos(theta_y), ((r_tot)^2/(r_w)^2)*I_w+I_b+m_a*l^2+m_w*(r_tot)^2];
...
x_xz=[theta_y; phi_y_do;t theta_y_dot];
u_xz=T_y;
